...
def load_pipeline_models():
    # Load stage 1 and stage 2 models from your final pipeline folder
    model_stage1 = load_model("models_stage2_pipeline/stage1_model.keras")
    model_stage2 = load_model("models_stage2_pipeline/stage2_model.keras")

    # Load metadata: encoder classes + best threshold
    with open("models_stage2_pipeline/metadata.pkl", "rb") as f:
...
def merged_pipeline(img_path, model_stage1, model_stage2, encoder_classes, best_thr, img_size=224):
    ...
    conf = {encoder_classes[i]: float(pred2[i]) for i in range(len(encoder_classes))}
...
def generate_pdf(disease, reco_text):
    pdf = FPDF()
    pdf.set_auto_page_break(auto=True, margin=10)
    pdf.add_page()
    pdf.set_font("Arial", size=12)

    pdf.multi_cell(0, 8, txt=f"Wheat Disease Diagnosis\n\nDisease: {disease}\n\nRecommendation:\n{reco_text}")
...
if uploaded:
    ...
    st.subheader(f"Model Diagnosis: **{disease}**")
    ...
    if disease == "Healthy":
        ...
        st.table({k: round(v, 4) for k, v in conf.items()})
